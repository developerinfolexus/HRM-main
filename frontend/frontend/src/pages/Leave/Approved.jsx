...
export default function Approved() {
  ...
  const fetchApprovedLeaves = async () => {
    try {
      const response = await leaveService.getLeaves({ status: 'Approved' });
      if (response.data.status === 'success') {
        setApprovedLeaves(response.data.data.leaves);
      }
      setLoading(false);
    } catch (error) {
      ...
      <div style={{ background: '#ffffff', borderRadius: 24, border: '1px solid #E6C7E6', boxShadow: '0 10px 30px -10px rgba(102, 51, 153, 0.1)', overflow: 'hidden' }} className="mt-3">
        <table className="table align-middle table-hover mb-0">
          ...
          <tbody>
              ...
              return (
                <tr key={l._id}>
                  ...
                  <td className="text-end pe-4">
                    <span className="badge px-3 py-2 rounded-pill shadow-sm" style={{ backgroundColor: '#E6C7E6', color: '#663399', border: '1px solid #66339930' }}>
            ...
            {approvedLeaves.length === 0 && !loading && (
              <tr>
                <td colSpan="5" className="text-center py-4 text-muted">No approved leaves found</td>
              </tr>
            )}
          </tbody>
        </table>
